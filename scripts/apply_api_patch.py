...
def protocol_autostart_script(protocol: str) -> str:
    ...
    return (
        ...
        f"network=$(ip -4 route show dev {iface} scope link | awk '$1 ~ /^[0-9]+[.]/ {{print $1; exit}}')\n"
        "egress=$(ip -4 route show default | awk '{for (i=1; i<NF; i++) if ($i == \"dev\") {print $(i+1); exit}}')\n"
        "if [ -n \"$network\" ] && [ -n \"$egress\" ]; then\n"
        "  iptables -t nat -C POSTROUTING -s \"$network\" -o \"$egress\" -j MASQUERADE 2>/dev/null || "
        "iptables -t nat -A POSTROUTING -s \"$network\" -o \"$egress\" -j MASQUERADE\n"
        "fi\n"
