...
CONFIG_PATH="${WG_CONFIG_PATH:-/opt/amnezia/wireguard/wg0.conf}"
INTERFACE="${WG_INTERFACE:-$(basename "$CONFIG_PATH" .conf)}"

echo "Container startup: 3WG WireGuard ($INTERFACE)"
...
[ -n "$network" ] || network="10.49.0.0/24"
