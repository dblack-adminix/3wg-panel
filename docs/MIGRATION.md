...
Также можно сделать это из web UI:
...
- Web UI autopush не переключает DNS сам. Это специально оставлено ручным шагом, чтобы не отрезать пользователей до проверки нового сервера.
