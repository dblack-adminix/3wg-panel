...
Если helper показывает старое локальное имя вроде `debian.debian`, `localhost` или `debian`, не оставляйте его. Для обычного сервера введите реальный домен в оба host-поля, например `nl-ams-08.nodax.eu`. Если VPN должен ходить через отдельный сменяемый IP/DNS, во второе поле введите отдельный endpoint-домен.
