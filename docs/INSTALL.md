...
- Для стабильной установки вместо `dev` можно указать tag, например `v1.1.4`, чтобы сервер не подтягивал текущие dev-изменения.
...
При выборе `1` installer установит Caddy через `apt-get`, добавит managed-блок в `/etc/caddy/Caddyfile` и направит домен на локальную панель `127.0.0.1:18080`. Установка пакетов выполняется в non-interactive режиме, поэтому Debian/Ubuntu не должны ждать ввода после `needrestart`.
